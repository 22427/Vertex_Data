...
bool Vertex::operator ==(const Vertex &o) const
{
	...
	const float eps = 200.0f*std::numeric_limits<float>::epsilon();
...
#endif
VertexData *VertexDataOPS::read_obj(std::ifstream& f)
{
	...
	while (std::getline(f,line))
	{
		...
		else if (type == "f")
		{
			...
			else if (normals.empty())	// positions and texcoords
			{
				for (int i = 0; i < 3; i++)
				{
					std::getline(line_str,vtx);
					sscanf(vtx.c_str(),"%d/%d",&p_id,&t_id);
			...
			else if (tex_coords.empty())	 // positions and normals
			{
				...
				{
					std::getline(line_str,vtx);
					sscanf(vtx.c_str(),"%d//%d",&p_id,&n_id);
			...
			else		// all three
			{
				...
				{
					std::getline(line_str,vtx);
					sscanf(vtx.c_str(),"%d/%d/%d",&p_id,&t_id,&n_id);
...
VertexData *VertexDataOPS::read_vd(std::ifstream& f)
{
//	if(!f)
//		return nullptr;
...
bool VertexDataOPS::write_vd(const VertexData *vd, std::ofstream& f)
{
//	if(!f)
//		return false;
//	union bd
...
VertexData *VertexDataOPS::read_ply(std::ifstream& f)
{
...
//		std::getline(line_str,op,' ');

//		if (op == "element")
//		{
//			std::string s;
//			std::getline(line_str,s,' ');
//			if (s == "vertex")
//			{
//				vertex_prop = true;
//				face_prop = false;
//				line_str>>vertex_count;
//			}
//			else if (s == "face")
//			{
//				face_prop = true;
...
//		{
//			std::string	res;
//			std::getline(line_str,res,' ');//list
//			std::getline(line_str,res,' ');// list length
//			std::getline(line_str,res,' ');// type of list data!
//			trim(res);
//			index_type = res;
//		}
...
VertexData *VertexDataOPS::read_off(std::ifstream& f)
{
...
bool VertexDataOPS::write_obj(const VertexData *vd, std::ofstream& f)
{
//	if(!f)
//		return false;
...
bool VertexDataOPS::write_ply(const VertexData *vd, std::ofstream&f)
{
//	if(!f)
//		return false;
//	fprintf(f,"ply\nformat ascii 1.0\n");
...
bool VertexDataOPS::write_off(const VertexData *vd, std::ofstream& f)
{
...
bool VertexDataOPS::write_to_file(
		const VertexData *vd,
		const std::string &p,
		VertexDataOPS::FileFormat f)
{
...
VertexData* VertexDataOPS::read_from_file(
...
bool VertexDataOPS::recalculate_normals(VertexData *vd, AttributeID to_attribute)
{
...
bool VertexDataOPS::recalculate_tangents(VertexData *vd, AttributeID to_attribute)
{
	if(!vd->vertex_configuration().has_attribute(to_attribute))
	{
		return false;
...
VertexData *VertexDataOPS::reconfigure(VertexData *data, const VertexConfiguration &new_config, bool null)
{
